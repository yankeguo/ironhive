...
- A list+watch loop maintains an in-memory map of pod states (phase, Ready condition, IP, allocation, lease deadline, template hash). A broken watch is re-established from a fresh list with backoff; a periodic reconcile is the safety net against missed events.
