...
`POST /agent/v1/shell` runs each command in a fresh bash — calls share nothing and run concurrently. bash is a hard dependency: both the state wrapper below and typical caller commands assume real bash syntax, so the agent probes it at boot (`bash` on its own `PATH`, supporting `trap ... EXIT`, really bash and not a POSIX-sh lookalike) and **refuses to start** without a working one — a sandbox image must provide it. The lookup uses the agent's process `PATH`, so a request's `env=PATH=...` does not affect it. The optional `cwd` field sets the working directory for that call only. The command's environment is assembled from two inputs:
